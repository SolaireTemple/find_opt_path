...
class MultipleExitsWidget(QWidget):
    ...
    def init_ui(self):
        ...
        btn_load = QPushButton("Загрузить карты входов (файлы file0.txt, file1.txt, ...)")
        btn_load.clicked.connect(self.load_matrices)
        layout.addWidget(btn_load)

        btn_layout = QHBoxLayout()
        self.btn_stage1 = QPushButton("Этап 1: Волны для каждого входа")
        ...
        self.info_label = QLabel("Загрузите карты (несколько файлов).")
    ...
    def start_stage1(self):
        ...
        for idx, mat in enumerate(self.anim_matrices):
            ...
            label = QLabel(f"Карта входа {idx+1} (шаг 0 из {steps})")
    ...
    def do_wave_step(self):
        ...
        for i, label in enumerate(labels):
            if i < len(self.anim_widgets):
                label.setText(f"Карта входа {i+1} (шаг {self.anim_current_step+1} из {self.anim_total_steps})")
    ...
    def load_matrices(self):
        filenames, _ = QFileDialog.getOpenFileNames(
            self, "Выберите карты для каждого входа", "", "Text files (*.txt)"
        )
        if len(filenames) < 2:
        ...
        for idx, mat in enumerate(self.matrices):
            col_layout = QVBoxLayout()
            col_layout.setAlignment(Qt.AlignCenter)
            label = QLabel(f"Исходная карта {idx+1}")
            label.setAlignment(Qt.AlignCenter)
            col_layout.addWidget(label)
            widget = MatrixWidget(mat)
            col_layout.addWidget(widget)
